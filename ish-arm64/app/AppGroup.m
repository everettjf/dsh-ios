...
NSURL *ContainerURL(void) {
    ...
    NSURL *url = nil;
    if (appGroup != nil)
        url = [NSFileManager.defaultManager containerURLForSecurityApplicationGroupIdentifier:appGroup];
    if (url == nil) {
        // No app group entitlement (e.g. a build signed without one): keep the
        // roots in the app's own container instead of failing to boot.
