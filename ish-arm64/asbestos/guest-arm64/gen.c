...
// Vector FP widening/narrowing conversions
extern void gadget_fcvtl_vec(void);        // FCVTL{2} Vd.4S/2D, Vn.4H/8H/2S/4S
extern void gadget_fcvtn_vec(void);        // FCVTN{2} Vd.4H/8H/2S/4S, Vn.4S/2D
extern void gadget_fcvtxn_vec(void);       // FCVTXN{2} Vd.2S/4S, Vn.2D
// Vector fixed-point conversions (shift-by-immediate class)
extern void gadget_scvtf_fixpt_vec(void);  // SCVTF Vd, Vn, #fbits
extern void gadget_ucvtf_fixpt_vec(void);  // UCVTF Vd, Vn, #fbits
extern void gadget_fcvtzs_fixpt_vec(void); // FCVTZS Vd, Vn, #fbits
extern void gadget_fcvtzu_fixpt_vec(void); // FCVTZU Vd, Vn, #fbits
...
static int gen_simd_fp(struct gen_state *state, uint32_t insn) {
    ...
    // Mask 0xffa01fe0 ignores imm8 (bits 20-13), Rd (bits 4-0), and type (bit 22).
    // (It used to be 0xffa07fe0, which kept imm8 bits 14-13 and let constants
    // whose low fraction bits are set fall through to a decoder without the
    // bit-6 inversion, so 2.25 became 0.140625.)
    if ((insn & 0xffa01fe0) == 0x1e201000) {
        uint32_t rd = insn & 0x1f;
    ...
    // SCVTF/UCVTF/FCVTZS/FCVTZU (vector, fixed-point) - AdvSIMD shift by immediate
    // 0 Q U 011110 immh immb opcode 1 Rn Rd; opcode 0x1c = [SU]CVTF, 0x1f = FCVTZ[SU]
    // immh=01xx: 32-bit lanes, immh=1xxx: 64-bit lanes (Q=1); fbits = 2*esize_bits - immh:immb
    if ((insn & 0x9f80fc00) == 0x0f00e400 || (insn & 0x9f80fc00) == 0x0f00fc00) {
        ...
        if (immh & 0xc) {
            ...
            uint32_t esize, fbits;
            if (immh & 0x8) { esize = 3; fbits = 128 - immhb; }
            else            { esize = 2; fbits = 64 - immhb; }
            if (esize == 3 && Q == 0) { gen_interrupt(state, INT_UNDEFINED); return 0; }
            void *gadget;
            if (opcode == 0x1c) gadget = U ? gadget_ucvtf_fixpt_vec : gadget_scvtf_fixpt_vec;
            else                gadget = U ? gadget_fcvtzu_fixpt_vec : gadget_fcvtzs_fixpt_vec;
            gen(state, (unsigned long) gadget);
            gen(state, rd | (rn << 8) | (fbits << 16) | (esize << 24) | (Q << 28));
            return 1;
        }
    }
...
skip_three_different:
    ...
    // FCVTL{2}/FCVTN{2}/FCVTXN{2} - AdvSIMD two-register misc widen/narrow
    // FCVTL:  0 Q 0 01110 0 sz 10000 10111 10 Rn Rd  (opcode=0x17)
    // FCVTN:  0 Q 0 01110 0 sz 10000 10110 10 Rn Rd  (opcode=0x16)
    // FCVTXN: 0 Q 1 01110 1 sz 10000 10110 10 Rn Rd  (opcode=0x16, sz must be 1)
    {
        uint32_t base = insn & 0xbffffc00;  // keep U, size, sz, opcode; drop Q
        void *gadget = NULL;
        if (base == 0x0e217800 || base == 0x0e617800) gadget = gadget_fcvtl_vec;
        else if (base == 0x0e216800 || base == 0x0e616800) gadget = gadget_fcvtn_vec;
        else if (base == 0x2e616800) gadget = gadget_fcvtxn_vec;
        if (gadget) {
            uint32_t Q = (insn >> 30) & 1;
            uint32_t rn = (insn >> 5) & 0x1f;
            uint32_t rd = insn & 0x1f;
    ...
    if ((insn & 0xff201fe0) == 0x1e201000) {
        ...
        uint32_t imm8 = ((insn >> 13) & 0xff) ^ 0x40;  // bit 6 is stored inverted
        uint32_t rd = insn & 0x1f;
